...
const ordenVentaCaja = JSON.parse(localStorage.getItem("ordenVenta"));
console.log(ordenVentaCaja);
const {ticker, precio, cantidad, numeroComitente} = ordenVentaCaja 

if (numeroComitente == usuarioLoggeado.numeroCcomitente) {
    
const nombreTicker = document.createElement("p");
nombreTicker.textContent = `Ticker: ${ticker}`;
contenedorCedearsord.appendChild(nombreTicker);

const precioElemento = document.createElement("p");
precioElemento.textContent = `Precio: ${precio}`;
contenedorCedearsord.appendChild(precioElemento);

const cantidadElemento = document.createElement("p");
cantidadElemento.textContent = `Cantidad: ${cantidad}`;
contenedorCedearsord.appendChild(cantidadElemento);
